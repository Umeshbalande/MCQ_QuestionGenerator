...
def get_completion(prompt, model="gpt-3.5-turbo",num_iterations=1):
...
def create_questions(prompt, repeat):
    output=[]
...
def create_csv(output):
    updated_res = seq(output).map(lambda each : convert_keys_to_lowercase(each)).to_list()
    #updated_res=output.to_list()
...
if __name__ == '__main__':
    ...
    questions_data = create_questions(prompt=prompt, repeat=1)
    filename = create_csv(questions_data)
